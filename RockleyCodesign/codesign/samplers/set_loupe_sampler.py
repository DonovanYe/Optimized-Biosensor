...
class Binarize(Function):
    ...
    @staticmethod
    def forward(ctx, rescaled_prob, budget):
        input_shape = rescaled_prob.size()
        u = torch.rand(input_shape[1:])
        if torch.cuda.is_available():
            u = u.cuda()
            rescaled_prob = rescaled_prob.cuda()
        rescaled_prob = rescaled_prob[0, :]
        while torch.sum(torch.le(u, rescaled_prob).float()) != budget:
            u = torch.rand(input_shape[1:])
            if torch.cuda.is_available():
                u = u.cuda()
...
class SetLOUPESampler(nn.Module):
    def __init__(self, input_size, budget):
        super(SetLOUPESampler, self).__init__()
    ...
    def forward(self, x):
        ...
        binary_map = self.binarize(rescaled, self.budget)
        nonzero = binary_map.nonzero()
        return x[:, torch.squeeze(nonzero)]
